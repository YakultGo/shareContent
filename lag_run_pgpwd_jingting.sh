...
#   本变体: PGPASSWORD 内联前缀传 psql（不 export）；Aurora SSL 可 export PGSSLMODE=require。
#   sysbench 在另一窗口自己跑，这边只测延迟。简写: lag_run.sh <master> <slave> [-D 秒]
set -uo pipefail

# 可选: 加载 PG 环境变量文件（缺失则跳过，使用脚本内默认值）
_env="$(dirname "$(readlink -f "$0")")/pg_replica_env.sh"
[ -f "$_env" ] && source "$_env" || true
...
选项:
  ...
  -W, --password PWD     PG 密码（默认 $PGPASSWORD，内联传 psql，不 export）
...
# 默认值 —— IP 必须由参数传入（位置参数或 -M/-S）
MASTER=""; SLAVE=""
PORT=5432; MPORT=""; SPORT=""
DB_USER="${PGUSER:-root}"
DB_PASS="${PGPASSWORD:-Taurus_123}"
R_DB_USER="${REPL_USER:-root}"
DB_NAME="${PGDATABASE:-test}"
INTERVAL=0.2
TIMEOUT_MS=30000
MAX_WAIT_MS=200
MAX_INFLIGHT=50
...
BIN="$(dirname "$(command -v psql 2>/dev/null || echo /usr/local/postgresql/bin/psql)")"
[ -x "$BIN/psql" ] || { echo "错误: 找不到 psql（PG 客户端未装或不在 PATH）" >&2; exit 1; }
...
# 密码内联前缀传 psql（不 export）——避免后台 fork 的轮询/计数子 shell 继承 PGPASSWORD，
# 也避免在全局环境里残留。env 变量 PGCONNECT_TIMEOUT/PGSSLMODE 仍透传给 psql。
_psql() { PGPASSWORD="$DB_PASS" PGCONNECT_TIMEOUT="${PGCONNECT_TIMEOUT:-10}" "$BIN"/psql "$@"; }
...
lag_check_one() {  # $1=key
    ...
    result=$(_psql -h"$SLAVE" -p"$SPORT" -U"$R_DB_USER" -d"$DB_NAME" \
...
# 建表/函数 + 等备机函数复制到 + 清残留
#    建表/函数失败（如 permission denied for schema public）会直接打印 psql 输出并退出，
#    不再静默吞掉——否则主循环 INSERT 只会报 "relation does not exist"，看不到根因。
setup() {
    local PG_OPTS="-h $MASTER -p $MPORT -U $DB_USER -d $DB_NAME -v ON_ERROR_STOP=1"
    local out rc
    out=$(_psql $PG_OPTS 2>&1 <<'SQL'
...
SQL
    ...
    rc=$?
    if [ "$rc" -ne 0 ]; then
        say "[lag_run] 建表/函数失败（master=$MASTER:$MPORT db=$DB_NAME user=$DB_USER）exit=$rc"
        say "$out"
        say "[lag_run] 常见原因: 用户对 public schema 无 CREATE 权限 —— 可在 master 上执行"
        say "[lag_run]   GRANT USAGE, CREATE ON SCHEMA public TO $DB_USER;"
        exit 1
    fi
    local ok=0 _
    for _ in $(seq 1 60); do
        if _psql -h "$SLAVE" -p "$SPORT" -U "$R_DB_USER" -d "$DB_NAME" \
    ...
    out=$(_psql $PG_OPTS -c "TRUNCATE TABLE lag_test;" 2>&1)
    if [ $? -ne 0 ]; then
        say "[lag_run] TRUNCATE 失败（master=$MASTER:$MPORT）"
        say "$out"
        exit 1
    fi
}
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    ...
    _psql -h"$MASTER" -p"$MPORT" -U"$DB_USER" -d"$DB_NAME" \
