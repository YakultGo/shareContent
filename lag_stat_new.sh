...
# 用法: lag_stats.sh <master_file> <delay_file> <out_txt> [sweep_start_ms sweep_end_ms] [long_lag_ceil_ms]
set -uo pipefail

if [ $# -lt 3 ]; then
  echo "Usage: $0 <master_file> <delay_file> <out_txt> [sweep_start_ms sweep_end_ms] [long_lag_ceil_ms]"
  exit 1
fi
...
awk -v master="$MASTER_FILE" -v diffs="$DIFFS" \
    -v wstart="$SWEEP_START" -v wend="$SWEEP_END" -v tceil="$TIMEOUT_CEIL" -v lceil="$LONG_LAG_CEIL" '
  BEGIN {
  ...
  }
  {
    ...
    if (rt == "TIMEOUT") {
      ...
      # 右删失：lag>=max_wait_ms 的 key 按截断值计入分布，不丢
      diff = lceil+0
      long_lag_n++
    } else if (rt ~ /^[0-9]+$/) {
      diff = (rt - mt)
...

{
  ...
  long_lag_rate=$(awk -v t="$long_lag_n" -v n="$total_n" 'BEGIN{ if(n>0) printf "%.2f%%", t*100/n; else print "0%" }')
  echo "# n=$total_n  timeout=$timeout_n($timeout_rate)  long_lag=$long_lag_n($long_lag_rate)"
  echo "# window=[${SWEEP_START:-none},${SWEEP_END:-none}] ms  timeout_ceil=${TIMEOUT_CEIL}ms  long_lag_ceil=${LONG_LAG_CEIL}ms"
  sort -n "$DIFFS" | awk -v tceil="$TIMEOUT_CEIL" -v lceil="$LONG_LAG_CEIL" '
    function pidx(n, p,   i) {
      i = int((p / 100.0) * n)
      if ((p / 100.0) * n > i) i++
      if (i < 1) i = 1
    ...
    END {
      ...
      min = v[1]; max = v[n]
      avg = sum / n
      printf "  n=%d  avg=%.2fms  med=%.2fms  P90=%.2fms  P95=%.2fms  P99=%.2fms  min=%.2fms  max=%.2fms\n", \
             n, avg, v[pidx(n,50)], v[pidx(n,90)], v[pidx(n,95)], v[pidx(n,99)], min, max
      if (max+0 >= tceil+0) print "  NOTE: max 触及右删失上限 " tceil "ms（存在超时 key，P99 含截断值）"
      if (max+0 >= lceil+0 && max+0 < tceil+0) print "  NOTE: max 触及 long_lag 截断上限 " lceil "ms（存在 LONG_LAG key，P99 含截断值）"
    }
  '
} > "$OUT_TXT"
